...
The two tiers do not draw exactly the same line, so GCC and Clang traces can differ
at the edges (documented in README's "Compiler-specific instrumentation"): user code
in namespace `std` (e.g. a `std::hash<Key>` specialization in the user's own TU) is
traced by GCC but dropped by Clang's name filter, while third-party header-only code
under `/usr/include` is excluded on GCC (`/usr/include` itself is on the discovered
list, and GCC matches entries as path substrings) but traced on Clang.
...
|-- misc/
|   |-- call-stack-logger-capture.gif      # Original demo capture (kept for reference)
|   |-- call-stack-logger-capture-new.gif  # Regenerated demo capture embedded in README
...
11. **Not traced by design:** hooks that fire before `trace_begin` (global
   constructors that happen to run first — cross-TU constructor order is not
   defined) or after `trace_shutdown` (later `atexit` handlers, static
   destructors) find `get_thread_fp()` returning nullptr and log nothing. Frames
   still on the stack when `exit()` is called never run their exit hook, so with
   `LOG_ELAPSED` their lines keep `[  pending ]` exactly like after a crash
   (README "What is not traced").
12. **Signal handlers must not be instrumented (known limitation):** the hooks
